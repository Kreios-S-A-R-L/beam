...
import { RouterInputs, RouterOutputs, api } from '@/server/utils/api'
...
const PostPage: NextPageWithAuthAndLayout = () => {
  ...
  const postQueryPathAndInput: RouterInputs['post']['detail'] = {
  ...
  const likeMutation = api.post.like.useMutation({
    onMutate: async (likedPostId) => {
      await utils.post.detail.cancel(postQueryPathAndInput)
...
function AddCommentForm({ postId }: { postId: number }) {
  ...
  const addCommentMutation = api.comment.add.useMutation({
    onSuccess: () => {
      return utils.post.detail.invalidate({
        id: postId,
      })
    },
    onError: (error) => {
